...
def roles_permitidos(*roles):
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if identity.get("tipo") not in roles:
...
def login_user():
    ...
    token = create_access_token(identity={"id": user.id, "tipo": "user"})
...
def login_paciente():
    ...
    if not paciente or not check_password(data["password"], paciente.password):
        return jsonify({"message": "Credenciales incorrectas"}), 401
    token = create_access_token(identity={"id": paciente.id, "tipo": "paciente", "nombre": paciente.nombre})
...
def login_medico():
    ...
    if not medico or not check_password(data["password"], medico.password):
        return jsonify({"message": "Credenciales incorrectas"}), 401
    token = create_access_token(identity={"id": medico.id, "tipo": "medico", "nombre": medico.nombre})
...
@roles_permitidos("medico")  # Ejemplo: solo médicos pueden crear laboratorios
...
def analiticas_por_paciente(paciente_id):
    identity = get_jwt_identity()
    if identity["tipo"] == "paciente" and identity["id"] != paciente_id:
        return jsonify({"message": "Acceso denegado"}), 403
